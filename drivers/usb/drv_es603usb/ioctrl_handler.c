...
#ifdef SPIDEV_H
#error finger print driver usb custom spi header
#endif
...
/*   	pr_debug("es603_response_verify i = %d", i);*/
		if (data[i] != target[i]) {
			pr_debug("%s:%s:%d Fail at data = %2X i = %d\n", __FILE__, __func__, __LINE__, data[i], i);
			break;
		}
	}
	++i;
...
int ES603_IO_Bulk_Read(usb_ss801u *dev, struct spi_ioc_transfer *u_xfers, unsigned n_xfers)
{
	int retval;
	int bytes_read = 0;
...
int ES603_IO_Bulk_Write(usb_ss801u *dev, struct spi_ioc_transfer *u_xfers, unsigned n_xfers)
{
	int retval;
	int bytes_read = 0;
	struct spi_ioc_transfer *u_tmp;
...
int es603_io_bulk_get_image(usb_ss801u *dev, struct spi_ioc_transfer *u_xfers, unsigned n_xfers)
{
	int retval;
...
int es603_io_bulk_get_full_image(usb_ss801u *dev, struct spi_ioc_transfer *u_xfers, unsigned n_xfers)
{
	int retval;
	int bytes_read = 0, image_size = 0;
	struct spi_ioc_transfer *u_tmp;
...
int Egis_IO_Bulk_Read(usb_ss801u *dev, int arg)
{
	...
	if (EGIS_SUCCESS(retval)) {
		...
		if (0 != retval) {
			if (_MaxBulkRead > Bulk_buffer.DataSize)
				memcpy(dataBuf, Bulk_buffer.Data, Bulk_buffer.DataSize);
			else
				EgisMsg(dev->bPrintDbgMsg, KERN_ERR, "=ioctl=EGIS_IOCTL_BULK_READ data too large\r\n");
