...
void ss801u_delete(struct kref *kref)
{
	...
	printk(KERN_INFO "Patrick=>ss801u_delete start\n");
	...
	printk(KERN_INFO "Patrick=>ss801u_delete finish\n");
}


static Device_Vendor Get_DeviceVendor(__le16 PID, __le16 VID)
{
	printk(KERN_INFO "Patrick=>Get_DeviceVendor start\n");
...
static int ss801u_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	printk(KERN_INFO "Patrick=>ss801u_probe start\n");

	printk(KERN_INFO "barry-> ss801u enter probe_function\n");
	...
	printk(KERN_INFO "BarryLai Testing\n");

	/* set up the endpoint information */
	/* use only the first bulk-in and bulk-out endpoints */
	iface_desc = interface->cur_altsetting;

	info("There are %d endpoint. \n", iface_desc->desc.bNumEndpoints);
	...
	/*usb_enable_autosuspend(dev->udev);*/ /* enable auto-suspend*/
	...
	info("SS801U device now attached to #%d\n", interface->minor);

	printk(KERN_INFO "Patrick=>ss801u_probe finish\n");
	return 0;
...
static void ss801u_disconnect(struct usb_interface *interface)
{
	...
	printk(KERN_INFO "Patrick=>ss801u_disconnect start\n");
	...
	printk(KERN_INFO "Patrick=>ss801u_disconnect finish\n");
...
static int ss801u_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	printk(KERN_INFO "Patrick->ss801u_suspend start\n");

	if (!dev) {
		printk(KERN_INFO "Patrick->=SUSPEND= dev is NULL\n");
		EgisMsg(dev->bPrintDbgMsg, KERN_ERR, "=SUSPEND= dev is NULL\r\n");
		return 0;
	}

	printk(KERN_INFO "Patrick=>ss801u device #%d now suspended\n", minor);
	...
	if (/*dev->bSetSuspendAsync &&*/ dev->async_suspend) {
		printk(KERN_INFO "Patrick-> call kill_fasync\n");
		kill_fasync(&dev->async_suspend, SIGIO, POLL_IN);
	}

	printk(KERN_INFO "Patrick-> call EgisSndContactModeCmd\n");
	retval = Egis_SndContactModeCmd(dev->dev_vendor, dev, CSW);

	printk(KERN_INFO "Patrick=>ss801u_suspend finish\n");
	mutex_unlock(&dev->io_mutex);
	return retval;
}
...
static int ss801u_resume(struct usb_interface *intf)
{
	...
	printk(KERN_INFO "Patrick=>ss801u_resume start\n");
	...
	printk(KERN_INFO "Patrick=>ss801u_resume finish\n");
	return 0;
}
...
static int ss801u_reset_resume(struct usb_interface *intf)
{
	...
	printk(KERN_INFO "Patrick=>ss801u_reset_resume start\n");
	...
	printk(KERN_INFO "Patrick=>ss801u_reset_resume finish\n");
...
{
	usb_ss801u *dev = usb_get_intfdata(intf);
	printk(KERN_INFO "Patrick=>ss801u_pre_reset start\n");
	...
	printk(KERN_INFO "Patrick=>ss801u_pre_reset finish\n");
...
{
	usb_ss801u *dev = usb_get_intfdata(intf);
	printk(KERN_INFO "Patrick=>ss801u_post_reset start\n");
...
printk(KERN_INFO "Patrick=>ss801u_post_reset finish\n");
    return 0;
}
...
static int __init usb_ss801u_init(void)
{
	int result;
	char *cid;
	printk(KERN_INFO "Patrick=>usb_ss801u_init start\n");
	printk(KERN_INFO "USB ss801u driver started");
	info(" v" VERSION_EGISDRIVER);

	board_get_cid_tag(&cid);
	if (strcmp(cid, "HTCCN701")) {
		printk(KERN_INFO "[USB]cid %s, disable finger print driver\n", cid);
		return -1;
	} else {
		printk(KERN_INFO "[USB]cid %s, enable finger print driver\n", cid);
	}
	...
	if (result) {
		...
		printk(KERN_INFO "USB ss801u driver OK");
	}

	printk(KERN_INFO "Patrick=>usb_ss801u_init finish\n");
	return result;
}

static void __exit usb_ss801u_exit(void)
{
	printk(KERN_INFO "Patrick=> usb_ss801u_exit start\n");
	info("\r\n ***** \r\n Driver Exit \r\n *****\r\n");
	usb_deregister(&ss801u_driver);

	printk(KERN_INFO "Patrick=> usb_ss801u_exit finish\n");
}
